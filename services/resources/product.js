...
class ProductResource {
  ...
  static async getProductSnapshot(req) {
    ...
    const snapshot = await redisClient.hgetall(snapshotKey)
    const existSnapshotCache = Boolean(Object.keys(snapshot).length)
    // if none

    if (!existSnapshotCache) {
      throw new APIError({ code: code.NOTFOUND, message: '找不到對應項目' })
    }
    // if yes
  ...
  static async postProducts(req) {
    ...
    const validation = await ProductToolKit.postProductsValidate(req)
  ...
  static async putProducts(req) {
    ...
    const validation = await ProductToolKit.putProductsValidate(req)
    const { result } = validation
    if (validation.error) {
      throw new APIError({ code: result.code, data: result.data, message: result.message })
    }
  ...
  static async getStock(req) {
    ...
    let resultProduct = {}
    const existStockCache = Boolean(Object.keys(product).length)
    if (!existStockCache) {
      throw new APIError({ code: code.NOTFOUND, message: '找不到對應項目' })
    }
  ...
  static async putStock(req) {
    ...
    const validation = await ProductToolKit.updateStockValidate(req)
    const { result } = validation
    if (validation.error) {
      throw new APIError({ code: result.code, data: result.data, message: result.message })
    }
    ...
    return { error: null, data: null, message: '修改成功' }
  }
}
